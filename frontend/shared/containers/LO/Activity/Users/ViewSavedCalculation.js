...
class ViewSavedCalculation extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  async componentDidMount() {
    if (this.props.match.path.includes('view-saved-calculation')) {
  ...
  render() {
    ...
    const data = checkCurrentRouteComponent
      ...
      : this.props.activityUserDetailes?.saved_calculation?.data
      ? this.props.activityUserDetailes?.saved_calculation?.data
      : [];

    const ViewsavedData = data.map((item, index) => ({
      key: index + 1,
              ...
              <Row style={rowStyle}>
                <Col flex="1 1 200px">
                  <HeaderBreadCrumb>
                    ...
                    <Title level={2}>View Saved Calculations</Title>
                  </HeaderBreadCrumb>
                </Col>
                  ...
                  <IsoWidgetsWrapper className="viewUserDetailActivity">
                    <IsoWidgetBox>
                      ...
                      <TableViews.SimpleView
                        isSelection={false}
                        columns={ViewsavedDataColumns}
                        dataSource={ViewsavedData}
                        isPaginate={
                          checkCurrentRouteComponent && {
                          ...
                          <Row gutter={[10]}>
                            <Col>
                              {!checkCurrentRouteComponent && (
                                <Link
                                  to={{
                                    pathname: `/portal/dashboard/activity/users/${this.props?.match?.params?.userdetail}/view-saved-calculation`,
                                  }}
                                >
                                  <Button type="tag">
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ViewSavedCalculation)
);
